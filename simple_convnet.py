...
def extract_data(filename, num_images):
	...
	with gzip.open(filename) as bytestream:
		bytestream.read(16)
		buf = bytestream.read(IMAGE_SIZE**2 * num_images * NUM_CHANNELS)
...
def error_rate(predictions, labels):
	return 1.0 - np.mean(np.equal(np.argmax(predictions,1), labels))
...
def main(_):
	...
	fc1_weights = tf.Variable(
		tf.truncated_normal([(IMAGE_SIZE // 4)**2 * 64, 512],
			stddev=0.1,
			seed=SEED,
			dtype=data_type()))
	...
	with tf.Session() as sess:
		...
		for step in xrange(int(num_epochs * train_size) // BATCH_SIZE):
			...
			if step % EVAL_FREQUENCY == 0:
				...
				start_time = time.time()
				print ('step %d (epoch %.2f), %.2f ms' %
					(step, float(step) * BATCH_SIZE / train_size,
						1000 * elapsed_time / EVAL_FREQUENCY))
				print('Minibatch loss: %.3f, learning rate: %.6f' % (l, lr))
				print('Minibatch error: %f' % error_rate(predictions, batch_labels))
				print('Validation error: %f' % error_rate(eval_in_batches(validation_data, sess), 
					validation_labels))
				sys.stdout.flush()
